BEGIN
    ...
    WHERE INS_DATETIME BETWEEN CONVERT(VARCHAR(8), ?, 112) AND DATEADD(DD, 1, CONVERT(DATETIME, ?))

    /* 문자열을 날짜로 변환 */
    SELECT *
    FROM WMT_COB_OUTBOUND_HR
    WHERE INS_DATETIME > CONVERT(DATETIME, ?)
      AND INS_DATETIME < DATEADD(DD, 1, CONVERT(DATETIME, ?))
